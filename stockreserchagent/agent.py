...
qualitative_agent = LlmAgent(
    ...
    tools=[google_search],
    instruction="""
    You are the Qualitative Analyst Agent. Your job is to assess the "Investible Grade Attributes" of a company. You deal with the non-numeric, soft aspects of the business. 
...
quantitative_agent = LlmAgent(
    ...
    tools=[google_search],
    instruction="""
    You are the Quantitative Screening Agent. Your goal is to analyze the financial health and efficiency of the company using the framework. 
...
valuation_agent = LlmAgent(
    ...
    tools=[google_search],
    instruction="""
    You are the Valuation Agent. Your job is to determine the "Intrinsic Value" of the company and apply a "Margin of Safety." 
...
async def generate_and_save_report(
    ...
    """
    Generates a Markdown file, saves it locally to the root directory, 
    saves it as an ADK artifact, and returns a message that triggers 
    the ADK UI to display the attachment.
    
    Args:
        report_content: The full Markdown content of the investment report.
        ticker_symbol: The stock ticker to use in the filename.
        tool_context: The ADK context object for artifact saving.
        
    Returns:
        A message that confirms the artifact was saved (triggers UI attachment).
    """
    
    filename = f"Investment_Report_{ticker_symbol}.md"
    
    # 1. Save the file locally (to the root directory)
    try:
        # The report_content is the Markdown text generated by the LLM
        with open(filename, 'w', encoding='utf-8') as f:
            f.write(report_content)
        print(f"Tool Success: Saved report locally as {filename}")
    except Exception as e:
        # Log the local file saving error but continue with artifact saving
        print(f"Tool Error: Could not save file locally to disk: {e}") 

    # 2. Save the file as an ADK Artifact for the chatbot UI
    try:
        # Convert string content to bytes for the ADK Part object
        ...
        # Save the artifact using the ADK context. This registers the file 
        # for attachment in the frontend UI.
        await tool_context.save_artifact(filename=filename, artifact=report_artifact)
        
        # This message will be returned to the agent and signals success, 
        # and the ADK runtime handles the attachment display.
        return f"Successfully generated report **{filename}** and attached it to the chat for download."
        
    except Exception as e:
        # Return the exception error message directly for easier debugging

        return f"Tool Error: Failed to create ADK artifact for the report: {e}"

# Tool definition
report_tool = FunctionTool(func=generate_and_save_report)
...
report_agent = LlmAgent(
    ...
    tools=[report_tool],
    instruction="""
    You are the Report Agent. Your responsibility is to collect the results from ALL previous agents (Qualitative Analyst, Quantitative Analyst, Valuation Analyst, and CIO Agent) and save them as a comprehensive Markdown report as well as send the saved report in chatbot UI using the `report_tool` tool.

    This report is MANDATORY for ALL tickers, regardless of pass/fail status.
    ...
    Once the final Markdown content is prepared, you MUST call the `save_report_artifact` tool with the ENTIRE Markdown content and the ticker symbol. Do display the report content in your final response; the tool will handle the output and attachment.
...
AgentWorkflow = SequentialAgent(
    name="AgentWorkflow",
    # We give the orchestrator access to the sub-agents as tools
    description=( 
            "A sequential workflow to Managing the iterative loop analyze a stock ticker through qualitative, quantitative, and valuation analysis, culminating in a"
...
orchestrator_agent = LlmAgent(
    name="lead_research_analyst",
    model=MODEL_NAME,
    # We give the orchestrator access to the sub-agents as tools
    sub_agents=[AgentWorkflow],
    instruction="""
    You are the Head of Research at a value investing firm. 
    You manage a team of analysts (Qualitative, Quantitative, Valuation).
    
    Your process for analyzing a stock ticker (e.g., "AMARAJABAT"):
    
    STEP 1: Ask QualitativeAnalyst to screen the business by analyzing its moat, management, and risks using "qualitative_agent" sub agent.
    - If QualitativeAnalyst reports "NO-GO" or "RED FLAG" status regarding corporate governance or business ethics, TERMINATE the analysis immediately. Issue a "REJECT" report citing qualitative risks. Do not waste compute on subsequent stages.
    
    STEP 2: If Stage 1 passes, ask QuantitativeAnalyst to run the 10-Point Checklist & analyse financial statements (P&L, Balance Sheet, Cash Flow) using "quantitative_agent" sub agent.
    - If the score is low on the '10-Point Checklist' (specifically high debt, negative operating cash flow, or ROE < 15%), TERMINATE the analysis. (e.g., < 6/10), issue a "REJECT" report citing weak fundamentals.
    
    STEP 3: If Stage 2 passes, ask ValuationAnalyst to perform a DCF valuation & Relative Valuation using "valuation_agent" sub agent.
    ...
    STEP 5: Generate a professionally formatted Markdown report using "report_agent" sub agent.
    ...
    - **Be extremely critical:** We only want to invest in the best businesses at the best prices
    - **Tone:** Professional, prudent, and conservative
    - **Priority:** Capital protection over speculation
    - **Language:** Only communicate in English; avoid jargon
    - **Decision Hierarchy:** Qualitative (business ethics) > Quantitative (financial health) > Valuation (price fairness)
    - **Agent Invocation:** Always explicitly invoke the named agent for each step (QualitativeAnalyst → QuantitativeAnalyst → ValuationAnalyst → CIO Agent → ReportAgent)
    - **Early Termination:** If any ticker fails Gate 1 or Gate 2, do NOT invoke ValuationAnalyst or CIO Agent for that ticker, but ALWAYS invoke ReportAgent for Step 5 to document the rejection
    - **Report Generation:** Step 5 (ReportAgent) is mandatory for ALL tickers, regardless of pass/fail status
    - **Prevent Hallucination:** Never generate data yourself. Always defer to the named agents. If an agent is not explicitly invoked, the analysis step is incomplete.
